...
const tier = rarityTiers.find((tier) => randomValue <= tier.accumulatedProbability) || rarityTiers[rarityTiers.length - 1];

// Filter creatures based on the rarity tier
const creaturesInTier = mythology.filter(
  (creature) => creature.rarity >= tier.minRarity && creature.rarity < tier.maxRarity
);


// If creaturesInTier is empty, use all creatures
const finalCreaturePool = creaturesInTier.length > 0 ? creaturesInTier : mythology;

// Randomly pick a creature from the filtered creatures
const randomCreature = finalCreaturePool[Math.floor(Math.random() * finalCreaturePool.length)];
