...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName('card').setDescription('The card you want to use.').setRequired(true)
    )
    .addStringOption((option) =>
      option.setName('attribute').setDescription('They attribute you want to use.').setRequired(true)
    )
  ...
  async execute(interaction) {
    ...
    collector.on('collect', async (interaction) => {
        if (interaction.customId === 'start') {
            ...
            const randomBotCard = mythology[Math.floor(Math.random() * mythology.length)];
          ...
          setTimeout(async () => {
            ...
            if (botAttribute > selectedAttribute) {
              ...
              await interaction.editReply(
                `I used the **${botName}** card with **${capitalizeFirstLetter(attributeName)}: ${botAttribute}** and won the battle! Your **${selectedName}** card had **${capitalizeFirstLetter(attributeName)}: ${selectedAttribute}**. You lost 50 :coin:!`
              );
              
            } else if (botAttribute == selectedAttribute) {
                await interaction.editReply(
                    `We both used the **${botName}** card with **${capitalizeFirstLetter(attributeName)}: ${botAttribute}** and tied!`
                  );
            }
             ...
             await userProfile.save().catch((err) => console.log(err));
              await interaction.editReply(
                `You won the battle! Your **${selectedName}** card with **${capitalizeFirstLetter(attributeName)}: ${selectedAttribute}** defeated my **${botName}** card with **${capitalizeFirstLetter(attributeName)}: ${botAttribute}**. You won 50 :coin:!`
              );
